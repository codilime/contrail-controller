...
void VectorToIp(const std::vector<int8_t> &ip, int family, IpAddress *sip,
    ...
    if (family == Address::INET) {
        assert(ip.size() >= 8);
        Ip4Address::bytes_type sbytes;
        Ip4Address::bytes_type dbytes;
        for (int i = 0; i < 4; i++) {
            sbytes[i] = ip.at(i);
            dbytes[i] = ip.at((i + 4));
        }
...
std::vector<int8_t> IpToVector(const IpAddress &sip, const IpAddress &dip,
    ...
    if (family == Address::INET) {
        Ip4Address::bytes_type sbytes = sip.to_v4().to_bytes();
        Ip4Address::bytes_type dbytes = dip.to_v4().to_bytes();
        ...
        Ip6Address::bytes_type sbytes = sip.to_v6().to_bytes();
        Ip6Address::bytes_type dbytes = dip.to_v6().to_bytes();
        std::vector<int8_t> ip_vect(sbytes.begin(), sbytes.end());
        std::vector<int8_t>::iterator it = ip_vect.begin();
...
void Ip6AddressToU64Array(const Ip6Address &addr, uint64_t *arr, int size) {
    ...
    if (size != 2)
        return;
    words = (const uint32_t *) (addr.to_bytes().data());
